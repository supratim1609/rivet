...
class ClientGenerator {
  ...
  String generate({
    ...
    // Note: Imports and headers are handled by source_gen or the user's main file
    ...
    buffer.writeln(_generateClientClass(className, routes, implements));
  ...
  String _generateClientClass(String className, List<RouteDefinition> routes, String? implements) {
    final buffer = StringBuffer();
  ...
  String _generateMethod(RouteDefinition route) {
    ...
    if (route.requestBodyType != null) {
      ...
      buffer.writeln();
      buffer.writeln(
        '    final response = await _client.$method(url, headers: headers);',
      );
    }
    ...
    if (route.responseType.startsWith('List<')) {
      final innerType = route.responseType.substring(
        5,
        route.responseType.length - 1,
      );
      ...
      buffer.writeln('      return ${route.responseType}.fromJson(json);');
    }
